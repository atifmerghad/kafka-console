...
import { headerData, responseConnect } from './sampleData';
...
const Connectors = () => {
  ...
  useEffect(() => {
    apiClient.get('http://localhost:8080/api/consumer-groups?clusterId=cluster1').then((response) => {
      ...
      var groups = responseConnect //response.data//.consumerGroups
      var count = 0;
      SetIsLoading(false)
      if (groups.length > 0) {
        groups.forEach((element, i) => {
          groups[i]['id'] = i; // element.groupId;
          count = 0;
        });
      }
      setPartitions(count);
  ...
  return (
    ...
    <div className="page-container">
      ...
      <Column lg={16} md={8} sm={4} className="landing-page__r2" style={{ marginBottom: '1rem' }}>
        <Tile lg={4} md={4} sm={4}>
          <Grid>
            <Column lg={2} md={8} sm={4}>Total Connectors <br />   {isLoading && <SkeletonText />} {!isLoading &&  <h3>  {groups.length}</h3>} </Column>
            <Column lg={2} md={8} sm={4}>Stable <br />  {isLoading && <SkeletonText />} {!isLoading &&  <h3>  {groups.length-1} </h3>}   </Column>
            <Column lg={6} md={8} sm={4}>connectors are unhealthy <br />  {isLoading && <SkeletonText />} {!isLoading &&  <h3> 1</h3>}   </Column>
          </Grid>
        </Tile>
      </Column>
