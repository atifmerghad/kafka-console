...
const Schemas = () => {
  ...
  useEffect(() => {
    apiClient.get('http://localhost:8080/api/consumer-groups?clusterId=cluster1').then((response) => {
      ...
      var groups = responseSchemas //response.data//.consumerGroups
      var count = 0;
      SetIsLoading(false)
      if (groups.length > 0) {
       /*
        groups.forEach((element, i) => {
          groups[i]['id'] = element.id //groupId;
          count = 0;
        });
        */
      }
  ...
  return (
    ...
    <div className="page-container">
      ...
      <Column lg={16} md={8} sm={4} className="landing-page__r2" style={{ marginBottom: '1rem' }}>
        <Tile lg={4} md={4} sm={4}>
          <Grid>
            <Column lg={2} md={8} sm={4}>Total Schemas <br />   {isLoading && <SkeletonText />} {!isLoading &&  <h3>  {groups.length}</h3>} </Column>
            <Column lg={2} md={8} sm={4}>Compatibility mode <br />  {isLoading && <SkeletonText />} {!isLoading &&  <h3>  Forward</h3>}   </Column>
          </Grid>
        </Tile>
      </Column>
