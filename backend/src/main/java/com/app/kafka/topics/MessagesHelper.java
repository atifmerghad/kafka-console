...
public class MessagesHelper {
    ...
    public List<MessageUtils.MessageHeader> mapHeaders(Headers headers) {
        List<MessageUtils.MessageHeader> result = new ArrayList<>();
        for (Header header : headers) {
            String headerValue = transformHeaderValue(header);
            result.add(MessageUtils.MessageHeader.builder()
                    .key(header.key())
                    .value(headerValue)
                    .build());
        }
    ...
    public void validateTopics(String serverId, List<String> topicNames) throws Exception {
        ...
        try {
            ...
            throw new Exception(String.format("Cannot check if topics [%s] exists on server [%s](%s)", topicNames, serverId, e.getMessage()));
        } catch (ExecutionException e) {
            throw new Exception(String.format("Cannot check if topics [%s] exists on server [%s](%s)", topicNames, serverId, e.getMessage()));
        }
        if (!topicsExists) {
            throw new Exception(String.format("Topics [%s] not exists on server [%s]", topicNames, serverId));
        }
    }
}
